#!/bin/sh
###############################################################################
# Restic Backup Stack - Pre-Deployment Validation & Setup
#
# Purpose: Validate prerequisites and prepare host paths for Restic.
...
log() {
    echo "[restic-validation] $*"
}

log_error() {
    echo "[restic-validation] ERROR: $*" >&2
}
...
ensure_dir_with_ownership "${APPDATA_PATH}/restic/secrets" "999:999" "750"

# Restic cache (written by restic as root)
ensure_dir_with_ownership "${APPDATA_PATH}/restic/cache" "root:root" "755"
