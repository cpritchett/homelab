#!/bin/sh
###############################################################################
# SMTP Relay Stack - Pre-Deployment Validation & Setup
#
# Purpose: Validate prerequisites and prepare host paths for the lightweight
#          internal SMTP relay stack.
# Tier: Platform (depends on Infrastructure tier)
#
# This script is IDEMPOTENT and safe to run before every deployment.
###############################################################################
...
log() {
    echo "[smtp-relay-validation] $*"
}

log_error() {
    echo "[smtp-relay-validation] ERROR: $*" >&2
}
...
ensure_dir_with_ownership "${APPDATA_PATH}/smtp-relay" "root:root" "755"
ensure_dir_with_ownership "${APPDATA_PATH}/smtp-relay/secrets" "999:999" "750"
