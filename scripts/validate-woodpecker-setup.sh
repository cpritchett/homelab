#!/bin/sh
###############################################################################
# Woodpecker CI Stack - Pre-Deployment Validation & Setup
#
# Purpose: Validate prerequisites and prepare host paths for Woodpecker.
# Tier: Platform (depends on Infrastructure tier + Forgejo)
#
...
log() {
    echo "[woodpecker-validation] $*"
}

log_error() {
    echo "[woodpecker-validation] ERROR: $*" >&2
}
...
# Woodpecker application data
ensure_dir_with_ownership "${APPDATA_PATH}/woodpecker" "root:root" "755"

# Secrets directory (written by 1password/op:2 as opuser 999:999)
ensure_dir_with_ownership "${APPDATA_PATH}/woodpecker/secrets" "999:999" "750"
