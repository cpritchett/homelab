#!/bin/sh
###############################################################################
# Reading Stack - Pre-Deployment Validation & Setup
#
# Purpose: Validate prerequisites and prepare host paths for
#          reading application stack (books, comics, manga, audiobooks).
...
log() {
    echo "[reading-validation] $*"
}

log_error() {
    echo "[reading-validation] ERROR: $*" >&2
}
...
# LSIO images (PUID/PGID 1701:1702)
for svc in kavita mylar3 bookshelf; do
    ensure_dir_with_ownership "${APPDATA_PATH}/reading/${svc}/config" "${MEDIA_OWNER}" "755"
done

# Komga runs as UID 1000 (non-LSIO)
ensure_dir_with_ownership "${APPDATA_PATH}/reading/komga/config" "1000:1000" "755"

# Audiobookshelf runs as root (non-LSIO)
ensure_dir_with_ownership "${APPDATA_PATH}/reading/audiobookshelf/config" "0:0" "755"
ensure_dir_with_ownership "${APPDATA_PATH}/reading/audiobookshelf/metadata" "0:0" "755"

# ReadMeABook (PUID/PGID 1701:1702)
ensure_dir_with_ownership "${APPDATA_PATH}/reading/readmeabook/config" "${MEDIA_OWNER}" "755"

# ReadMeABook Redis
ensure_dir_with_ownership "${APPDATA_PATH}/reading/readmeabook/redis" "999:999" "755"

# Secrets directory for 1Password hydration
ensure_dir_with_ownership "${APPDATA_PATH}/reading/secrets" "999:999" "750"

# ---------------------------------------------------------------------------
# Reading data directories (data01 — bulk tier)
# ---------------------------------------------------------------------------
