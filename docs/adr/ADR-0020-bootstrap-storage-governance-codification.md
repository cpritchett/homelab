# ADR-0020: Bootstrap, Storage, Repository Governance, and NAS Stacks Codification

**Status:** Accepted  
**Date:** 2025-12-23 (Updated: 2025-12-26)  
...
5. **Add `stacks/` root directory** for NAS deployment manifests (Docker Compose, systemd services, etc.) targeting non-Kubernetes nodes.
...
- NAS deployment manifests have a dedicated, first-class location separate from Kubernetes manifests, preventing confusion between K8s and non-K8s workloads.
...
- **Place NAS stacks under `infra/` or `kubernetes/`** — rejected; NAS nodes are not Kubernetes nodes, and mixing deployment targets causes confusion and tooling mismatches.
